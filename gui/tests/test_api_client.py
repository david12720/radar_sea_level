...
def test_result_keys_present(client):
    r = client.query(range_m=500, azimuth_deg=0, elevation_deg=0, terrain_msl_m=0.0)
    for key in ("lat_deg", "lon_deg", "alt_msl_m", "terrain_msl_m", "agl_m", "horiz_range_m"):
...
def test_validation_range_too_large(client):
    with pytest.raises(RuntimeError):
        client.query(range_m=99999, azimuth_deg=0, elevation_deg=0, terrain_msl_m=0.0)


def test_validation_bad_azimuth(client):
    with pytest.raises(RuntimeError):
        client.query(range_m=1000, azimuth_deg=360, elevation_deg=0, terrain_msl_m=0.0)


def test_validation_bad_elevation(client):
    with pytest.raises(RuntimeError):
        client.query(range_m=1000, azimuth_deg=0, elevation_deg=50, terrain_msl_m=0.0)
