...
def _target_popup(t: dict) -> html.Div:
    ...
    return html.Div([
        ...
        html.Table([
            ...
            row("Terrain MSL", f"{t['terrain_msl_m']:.1f} m"),
            ...
            *([ row("Open-Meteo (COP)",
                    f"{t['open_meteo_elevation_m']:.1f} m"
                    if t.get('open_meteo_elevation_m') is not None else "N/A") ]
              if "open_meteo_elevation_m" in t else []),
            *([ row("Open-Topo (SRTM30)",
                    f"{t['open_topo_elevation_m']:.1f} m"
                    if t.get('open_topo_elevation_m') is not None else "N/A") ]
              if "open_topo_elevation_m" in t else []),
            *([ row("Open-Elev (SRTM)",
                    f"{t['open_elevation_m']:.1f} m"
                    if t.get('open_elevation_m') is not None else "N/A") ]
...
def _radar_popup(r: dict) -> html.Div:
    ...
    return html.Div([
        ...
        html.Table([
            ...
            row("Terrain MSL", f"{r.get('terrain_msl_m', 0):.1f} m MSL"),
