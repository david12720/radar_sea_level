...
```bash
wsl bash -c "cd /mnt/c/Users/david/Downloads/radar_sea_level && g++ -std=c++17 -O2 -Iinclude -o radar_sea_level main.cpp src/dted_tile.cpp src/dem_database.cpp src/elevation_lut.cpp src/radar_compute.cpp"
```

Or with CMake:
```bash
wsl bash -c "cd /mnt/c/Users/david/Downloads/radar_sea_level && mkdir -p build && cd build && cmake .. && make"
```

Run:
```bash
wsl bash -c "cd /mnt/c/Users/david/Downloads/radar_sea_level && ./radar_sea_level"
```
...
35 SRTM3 `.hgt` files in `tiles/`, covering Israel (lat 28–35, lon 30–35).  
